...
class SolveBondQTSM(object):
    ...
    def __init__(self, sess):
        ...
        self.d = 3
        self.T = 0.3
        self.A = tf.constant([-1.7318, 0, 0, 0.0043, -0.0037, 0, 2.7202, -1.1898, -0.3411], shape=(3,3), dtype=tf.float64)
        self.B = tf.constant([0.0608, 0.0007, 0.1343], shape=(3,1), dtype=tf.float64)
        self.sigma = tf.constant([1.5143, 0, 0, 0, 0.1543, 0, 0, 0, 0.5916], shape=(3,3), dtype=tf.float64)
        self.Gamma = tf.constant([1, -0.5847, -0.3866 , 0, 1, 0.9101, 0, 0, 1], shape=(3,3), dtype=tf.float64)
        self.R = tf.constant([0, 0, 0], shape=(3,1), dtype=tf.float64)
        self.k = 0.0338
        self.X0 = tf.constant([0,0,0], shape=(1, self.d), dtype=tf.float64)
        # parameters for algo
        self.n_time = 20
        self.n_layer = 4
        self.n_neuron = [self.d, self.d + 5, self.d + 5, self.d]
        self.batch_size = 64
        self.valid_size = 256
        self.n_maxstep = 4
        self.n_displaystep = 20
        ...
        self.allones_m = tf.ones(shape=tf.stack([tf.shape(self.dW)[0], 1]), dtype=tf.float64)
        self.allones_d = tf.ones(shape=(1,self.d), dtype=tf.float64)
        self.B_ext = tf.matmul(self.allones_m, self.B, transpose_b=True)
        self.k_ext = self.k * self.allones_m
    ...
    def train(self):
        ...
        feed_dict_valid = {self.dW: dW_valid, self.is_training: False}
        ...
        for _ in range(self.n_maxstep + 1):
            ...
            self.sess.run(self.train_op, feed_dict={self.dW: dW_train, self.is_training: True})
    ...
    def build(self):
        ...
        X = tf.matmul(self.allones_m, self.X0)
        Y = self.allones_m * self.Y0
        X_new = tf.matmul(self.allones_m, self.X0)
        Y_new = self.allones_m * self.Y0
        Z = tf.matmul(self.allones_m, self.Z0)
        with tf.variable_scope('forward'):
            for t in range(0, self.n_time - 1):
                Y_new = Y - self.f_tf(self.t_stamp[t], X, Y, Z) * self.h \
                            + tf.reduce_sum(Z * self.dW[:, :, t], 1, keepdims=True)
                X_new = X + self.b_tf(t, X, Y, Z) * self.h + tf.matmul(self.dW[:, :, t], self.sigma, transpose_b=True)
                X = X_new
                Y = Y_new
                Z = self._one_time_net(X, str(t + 1)) / self.d
            ...
            X = X_new
            Y = Y_new
            term_delta = Y - self.g_tf(self.T, X)
    ...
    def g_tf(self, t, X):
        # terminal conditions
        return self.allones_m

    def f_tf(self, t, X, Y, Z):
        # nonlinear term 1
        return Y * (tf.reshape(tf.diag_part(tf.matmul(X, tf.matmul(self.Gamma, X, transpose_b=True))), shape=tf.stack([tf.shape(self.dW)[0], 1])) \
                    + tf.matmul(X, self.R) + self.k_ext) \
               + tf.reshape(tf.diag_part(tf.matmul(Z, Z, transpose_b=True)), shape=tf.stack([tf.shape(self.dW)[0], 1])) / Y

    def b_tf(self, t, X, Y, Z):
        # nonlinear term 2
        return tf.matmul(X, self.A, transpose_b=True) + self.B_ext + tf.matmul(Z, self.sigma, transpose_b=True) / tf.matmul(Y, self.allones_d)
...
def main():
    ...
    with tf.Session() as sess:
        ...
        np.savetxt("./QTSM_d3.csv", output, fmt=['%d', '%.5e', '%.5e'], delimiter = ",", \
